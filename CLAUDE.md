...
- State er versioneret (`version: 7`). Enhver ny/ændret datastruktur kræver et migreringstrin i `migrate()` OG defaults i `normalize()` — gamle backups og den delte live-state skal altid kunne indlæses.
...
- Ud over selve state bruger appen tre localStorage-nøgler ved siden af: `rtd-viewer-mode` (visningstilstand), `rtd-last-export` (seneste backup, driver påmindelsen) og `rtd-conflict` (ugemte ændringer henlagt ved konflikt). De hører til enheden, ikke klubbens data, og må derfor ikke ind i state — de kræver heller ingen migrering.
- Ved `conflict` fra `artifactNS.publish()` genindlæser runtimen selv alle visninger til vinderens udgave. Der må aldrig forsøges igen automatisk; i stedet henlægges vores state (`stashConflict()`), så den kan hentes frem efter genindlæsningen.
