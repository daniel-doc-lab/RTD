...
(function () {
  ...
  function normalize(s) {
    ['members', 'fineTypes', 'clubYears', 'meetings', 'fines', 'payments'].forEach(function (k) {
  ...
  function viewYearDetail() {
    ...
    return '<div class="meet-head">' +
      ...
      rows +
      '<div class="actionbar"><button class="btn secondary" data-action="add-meeting" data-id="' + esc(y.id) + '">' + IC.plus + 'Tilføj ekstra møde</button></div>';
  }
  ...
  function viewMeeting() {
    ...
    var grid = members.map(function (m) {
      ...
      return '<button class="member-cell' + (sum > 0 ? ' hit' : '') + '" data-action="pick-fines" data-id="' + esc(m.id) + '"' + (meet.closedAt ? ' disabled style="opacity:0.55"' : '') + '>' +
    ...
    var log = meetFines.slice().reverse().map(function (f) {
      ...
      return '<div class="log-item">' +
        ...
        (meet.closedAt ? '' : '<button class="x" data-action="remove-fine" data-id="' + esc(f.id) + '" aria-label="Fjern bøde">' + IC.x + '</button>') +
        '</div>';
    }).join('');
    ...
    return '<div class="meet-head">' +
      ...
      '<button class="iconbtn" data-action="edit-meeting" data-id="' + esc(meet.id) + '" aria-label="Rediger møde">' + IC.edit + '</button>' +
  ...
  function openFinePicker(memberId) {
    ...
    var grid = state.fineTypes.filter(function (t) { return t.active; }).map(function (t) {
      return '<button class="fine-btn' + (t.amount >= HOT_FINE ? ' hot' : '') + '" data-action="give-fine" data-id="' + esc(t.id) + '">' +
  ...
  function openMemberSheet(memberId) {
    ...
    state.payments.forEach(function (p) {
      ...
      events.push({ ts: p.ts, html: '<div class="log-item"><div class="grow"><div class="t">Indbetaling' + (p.note ? ' — ' + esc(p.note) : '') + '</div>' +
        ...
        '<button class="x" data-action="remove-payment" data-id="' + esc(p.id) + '" aria-label="Slet indbetaling">' + IC.x + '</button></div>' });
    });
      ...
      input.onchange = function () {
        ...
        reader.onload = function () {
          try {
            ...
            if (!s || typeof s.version !== 'number' || s.version < 1 || s.version > 3 || !Array.isArray(s.members)) throw new Error('bad');
